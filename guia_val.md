...
- el [**Text refós de la Llei de Propietat
Intel·lectual**](https://www.boe.es/buscar/act.php?id=BOE-A-1996-8930) (**TRLPI**) dóna un tractament específic a aquest tipus d'obres que permet la seva publicació en un context educatiu
...
equitativa a la qual dóna dret la llei.
...
qualsevol forma (article 2 del TRLPI) i pot transmetre'l per escrit
...
parcial d'obres impreses (o susceptibles de ser-ho)* amb fins educatius. La
reproducció s'ha de limitar a fragments consistents en:
...
equitativa ha de ser gestionada, necessàriament, per una *entitat de gestió* de
...
autorització de l'autor i si la seva reproducció genera obligacions econòmiques
per a la universitat. En certs casos, el professorat ha de proporcionar
informació que identifiqui inequívocament l'obra o fragment de obra utilitzada.
...
qualsevol fitxer només a l'efecte de comprovar que s'efectua una correcta
...
Punxant en el botó "Iniciar revisió" s'accedeix a la pàgina principal del
mòdul GDPI. Des d'ella podem iniciar el procés de gestió de cada document
i comprovar el seu estat.
...
correspondre amb obres impreses o susceptibles de ser-ho. Sota el llistat es
mostren opcions per filtrar la relació de documents que es mostren en el
mateix.
...
- **Estat**: És l'estat de tramitació de la gestió de drets de propietat intel·lectual.

Acompanya a cada document, a més, la possibilitat d'efectuar dues accions:
...
- *tots els documents* el tipus permet deduir que és una obra impresa o susceptible de ser-ho,
...
resposta es rebrà per correu electrònic, de manera que el mòdul GDPI no fa
un seguiment d'aquest procés.
...
d'avaluació, llistats amb els resultats d'avaluació d'estudiants,
etcètera) es poden publicar sense necessitat d'obtenir cap autorització.
Aquests documents són propis de la universitat i queden fora de l'àmbit de la
gestió de drets de propietat intel·lectual.

Si s'escull aquesta opció, no cal proporcionar dades relatives al
document. Premi l'opció "Tramitar" per processar el document.

El contingut del document no pot ser consultat per les entitats de gestió
com a part d'un procés d'auditoria, ja que pot contenir informació de
caràcter personal.
...
- **Nombre de matriculats**: El nombre de matriculats es genera automàticament a partir del nombre d'inscrits al curs. Atès que aquest nombre pot variar en el temps o ser incorrecte en el moment de publicació del document, el seu valor pot editar perquè reflecteixi la realitat a partir de l'experiència del professorat en edicions anteriors del curs (encara que sigui de manera aproximada).
- **Observacions**: Camp de format lliure amb un comentari que acompanyarà el document.

El fragment es pot publicar sense demanar autorització i l'obligació
econòmica que genera el seu ús ha estat satisfeta en el marc del conveni
celebrat entre CRUE i CEDRO-VEGAP.
